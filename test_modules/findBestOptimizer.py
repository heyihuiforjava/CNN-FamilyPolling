...
def stopTraning(score_liste):
    try:
        return score_liste[-1][1] <= score_liste[-2][1] <= score_liste[-3][1] <= score_liste[-4][1]
...
folder_to_save_model = "..\\models\\best_parameter\\"
file_to_save_model = "Optimizer_Analysis_my_%s_model" % model_to_use
...
for optimizer in ['sgd', 'rmsprop', 'adagrad', 'adadelta', 'adam', 'adamax', 'nadam']:
    ...
    # # Change only the model optimizer :
    model.compile(loss=model.loss,
                  optimizer=optimizer,
                  metrics=model.metrics)

    model.save(folder_to_save_model + file_to_save_model + "_optimizer_" + str(optimizer) + "_.h5")
    ...
    while score[1] <= .95 and compteur < 100:
        ...
        model.fit(x_train, y_train, batch_size=32, epochs=1, verbose=0)  # train model
        ...
        # saving it to disk
        model.save(file_to_save_model + "_optimizer_" + str(optimizer) + "_.h5")
    ...
    time_vs_optimizer[str(optimizer)] = time_liste
    score_vs_optimizer[str(optimizer)] = score_liste
    del model
    K.clear_session()
    print("###########################################")

save_obj(time_vs_optimizer, "time_vs_optimizer.txt")
save_obj(score_vs_optimizer, "score_vs_optimizer.txt")
