...
file_to_save_model = "Losses_Analysis_my_%s_model" % model_to_use
...
for loss in ['mean_squared_error', 'mean_absolute_error', 'squared_hinge', 'logcosh', 'binary_crossentropy', 'poisson']:
    compteur = 0
    score_liste = []
    time_liste = []
    print("Losses = ", loss)
    ...
    # # Change only the model loss :
    model.compile(loss=loss,
                  optimizer=model.optimizer,
                  metrics=model.metrics)

    model.save(file_to_save_model + "_loss_" + str(loss) + "_.h5")
    ...
    time_vs_losses[str(loss)] = time_liste
    score_vs_losses[str(loss)] = score_liste
    del model
    K.clear_session()
    print("###########################################")

save_obj(time_vs_losses, "time_vs_losses.txt")
save_obj(score_vs_losses, "score_vs_losses.txt")
